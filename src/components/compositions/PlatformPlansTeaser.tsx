...
export async function PlatformPlansTeaser() {
  ...
  return (
    <section id="pricing-teaser" className="bg-white py-12 md:py-16">
      <div className="mx-auto max-w-[1200px] px-5 md:px-8">
        <header className="mx-auto mb-8 max-w-[640px] text-center md:mb-10">
          ...
          <p className="text-[14px] leading-[22px] text-muted md:text-[16px]">
            From getting discovered online to a complete digital clinic with marketing — pick the tier that fits, with a 14-day free trial.
          </p>
        </header>
        ...
        {plans.length === 0 ? (
          ...
          <div className="mx-auto grid max-w-[900px] grid-cols-1 items-stretch gap-5 sm:grid-cols-3">
            {plans.map((p) => (
              <article
                key={p.id}
                className={
              ...
              >
                ...
                <p className="mt-1 min-h-[18px] text-[11px] text-muted">
                  {p.included_doctor_seats} seat{p.included_doctor_seats === 1 ? "" : "s"} included
                </p>
                <Link
                  href={`/get-started?plan=${p.code}`}
                  className={
                    "mt-auto inline-flex w-full items-center justify-center rounded-md px-4 py-2 text-[13px] font-medium no-underline " +
                    (p.is_popular
